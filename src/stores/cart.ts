...
export const useCartStore = defineStore('cart', () => {
   ...
   * Unlike the cash path this does NOT clear the cart — we keep the cart
   * intact until the card actually succeeds, so a decline/cancel leaves the
   * order recoverable and the cashier can retry or switch to cash. CartView
   * clears on confirmed payment.
   *
   * Throws (ApiError) if offline or the create fails — card is online-only
