...
export function selectionLabels(line: CartLine): string[] {
  ...
  for (const group of line.item.optionGroups) {
    for (const choiceId of line.selections[group.id] ?? []) {
      const choice = group.choices.find((c) => c.id === choiceId);
      if (!choice) continue;
      labels.push(`${group.label}: ${choice.label}`);
    }
  }
  return labels;
}
...
/**
 * Collapse a configured line into the PosLineItem the order API accepts.
 * Option price deltas are folded into unitPriceCents; choice labels go in the
 * structured `modifiers` array. The name stays CLEAN (no parenthetical) so POS
 * orders persist identically to web/storefront orders. `selectionSummary` is
...
export function defaultSelections(
  ...
  for (const group of item.optionGroups) {
    sel[group.id] = group.choices
      ...
      .map((c) => c.id);
    // SINGLE with no surviving default: pick first available.
...
export function validateSelections(
  ...
  for (const group of item.optionGroups) {
    ...
    if (
      ...
      return `${group.label} allows one choice`;
    }
    if (
      group.selectionType === 'MULTI' &&
      group.maxSelections > 0 &&
      picks.length > group.maxSelections
    ) {
      return `${group.label}: choose at most ${group.maxSelections}`;
    }
  }
  return null;
}
